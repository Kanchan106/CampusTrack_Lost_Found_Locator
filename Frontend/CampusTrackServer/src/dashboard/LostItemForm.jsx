...
import ReportFoundForm from "./FoundItemForm";
...
export default function ReportLostForm() {
  ...
  if (showFoundForm) return <ReportFoundForm />;

  return (
    <div className="form-wrapper">
      <div className="toggle-switch">
        <button className="active">Report Lost</button>
        <button onClick={() => setShowFoundForm(true)}>Report Found</button>
      </div>

      <div className="form-box lost-theme">
        <h3>Report Lost Item</h3>
        <form onSubmit={handleSubmit}>
          <div className="form-group"><label>Item Name</label><input name="name" onChange={handleChange} required /></div>
          <div className="form-group"><label>Description</label><textarea name="description" rows="5" onChange={handleChange} required /></div>
          ...
          <div className="form-group"><label>Image</label><ImageUpload onFileSelect={(file) => setFormData((prev) => ({ ...prev, file }))} /></div>
